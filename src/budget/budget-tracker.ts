...
export class BudgetTracker {
  ...
  /**
   * Restore state from persisted data (for crash recovery).
   * Directly sets internal state including Anthropic counters.
   */
  restoreState(restoredState: {
    ...
    this.state.totalSpendUSD = restoredState.totalSpendUSD;
    this.state.stageSpend = { ...restoredState.stageSpend };
    this.state.anthropicCalls = restoredState.anthropicCalls;
    this.state.anthropicSpend = restoredState.anthropicSpend;

    // Create a single synthetic call entry for tracking purposes
    // This preserves the spend data without creating misleading call history
    if (restoredState.totalSpendUSD > 0) {
      this.state.callHistory.push({
        timestamp: new Date().toISOString(),
        stage: 'deliberation',
        cost: {
          inputTokens: 0,
          outputTokens: 0,
          costUSD: 0, // Don't double-count spend
          tier: 'openai-mini',
          model: 'restored',
          provider: 'restored',
        },
        reasonCode: 'restored_from_persistence',
      });
    }
  }
}
